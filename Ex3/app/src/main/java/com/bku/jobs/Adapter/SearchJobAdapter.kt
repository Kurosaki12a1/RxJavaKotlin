...
class SearchJobAdapter(context: Context, lst: ArrayList<JobData>) : RecyclerView.Adapter<SearchJobAdapter.ViewHolder>() {
    private var lstJobInfo: ArrayList<JobData>
    ...
    override fun onBindViewHolder(holder: SearchJobAdapter.ViewHolder, position: Int) {
        ...
        Glide.with(ctx).load(jobInfo.companyLogo).into(holder.companyLogo)
        holder.titleJob.text = jobInfo.title
        holder.typeJob.text = "/ " + jobInfo.type
        holder.location.text = jobInfo.location
